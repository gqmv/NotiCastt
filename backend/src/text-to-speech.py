    ...
    try:
        ...
        for i, voice_name in enumerate(voices):
            ...
            if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                print(f"Synthesis finished for {voice_name}. Audio saved to {audio_file_path}")
                # Adicionar ao segmento de áudio
                audio_segments.append(AudioSegment.from_wav(audio_file_path))
            elif result.reason == speechsdk.ResultReason.Canceled:
                cancellation_details = result.cancellation_details
                print(f"Speech synthesis canceled for {voice_name}: {cancellation_details.reason}")
        ...
        if len(audio_segments) > 1:
            ...
            combined_audio.export(os.path.join(output_folder, "combined_audio.wav"), format="wav")
            print(f"Combined audio saved to {os.path.join(output_folder, 'combined_audio.wav')}")
...
voices_to_generate = ["pt-BR-AntonioNeural", "pt-BR-BrendaNeural", "pt-BR-GiovannaNeural"]
